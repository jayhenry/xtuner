...
from xtuner.v1.config import AdamWConfig, FSDPConfig, MuonConfig
...
class TestMoonEPPersistence(DeterministicDDPTestCase):
    ...
    def _assert_optimizer_dcp_cold_resume(self, optim_cfg) -> None:
        checkpoint_root = _shared_temporary_directory()
        weights_dir = checkpoint_root / "weights"
        torch.manual_seed(20260805)
        reference = self._build_engine(optim_cfg)
        ...
        try:
            ...
            resumed = self._build_engine(optim_cfg)
            resumed.init_model_weights()
            resumed.load_dcp(weights_dir)
            actual_checkpoint_optimizer = _optimizer_tensor_state(resumed)
            missing = checkpoint_optimizer.keys() - actual_checkpoint_optimizer.keys()
            unexpected = actual_checkpoint_optimizer.keys() - checkpoint_optimizer.keys()
            assert not missing and not unexpected, (
                f"missing optimizer states: {sorted(missing)}; unexpected optimizer states: {sorted(unexpected)}"
            )
            for name, expected in checkpoint_optimizer.items():
                torch.testing.assert_close(
                    actual_checkpoint_optimizer[name],
                    expected,
                    rtol=0,
                    atol=0,
                    msg=f"checkpoint optimizer state mismatch: {name}",
                )
            ...
            for name, expected in expected_model.items():
                torch.testing.assert_close(actual_model[name], expected, rtol=0, atol=0)
            ...
            if not reference._closed:
                reference.close()
            if resumed is not None and not resumed._closed:
                resumed.close()
            dist.barrier()
            if dist.get_rank() == 0:
    ...
    def test_async_dcp_snapshot_survives_concurrent_step_and_close(self) -> None:
        self.create_pg("cuda")
        checkpoint_root = _shared_temporary_directory()
        weights_dir = checkpoint_root / "weights"
        torch.manual_seed(20260805)
        ...
        try:
            ...
            save_future = reference.async_save_dcp(weights_dir)
            expected_loss, expected_norm = _optimizer_step(reference, offset=32)
            expected_model = _local_model_state(reference)
            expected_optimizer = _optimizer_tensor_state(reference)

            # close() owns pending persistence work; callers need not race the
            # writer with VMM unmap or auxiliary process-group destruction.
            reference.close()
            ...
            if not reference._closed:
                # The failing lifecycle implementation may have destroyed the
                # async PG already; wait externally before collective cleanup.
    ...
    def test_swap_adamw_sync_dcp_cold_resume(self) -> None:
        self.create_pg("cuda")
        self._assert_optimizer_dcp_cold_resume(AdamWConfig(foreach=False, swap_optimizer=True))

    def test_muon_sync_dcp_cold_resume(self) -> None:
        self.create_pg("cuda")
        self._assert_optimizer_dcp_cold_resume(MuonConfig())
