"""Test: does adding cuda sync after reduce_scatter fix the non-determinism?"""
import argparse, os
import numpy as np
import torch
...
parser.add_argument("--sync-mode", choices=["none", "ag", "rs", "both"], default="none")
parser.add_argument("--save-dir", default="/tmp/rs_sync_probe")
...
def _patched_ag(*a, **kw):
    result = _orig_ag(*a, **kw)
    if args.sync_mode in ("ag", "both"):
        torch.cuda.synchronize()
    return result

def _patched_rs(*a, **kw):
    result = _orig_rs(*a, **kw)
    if args.sync_mode in ("rs", "both"):
        torch.cuda.synchronize()
    return result

dist.all_gather_into_tensor = _patched_ag
...
fully_shard(mha, mesh=mesh, mp_policy=mp_policy, reshard_after_forward=True, offload_policy=None)

if rank == 0:
    print(f"\n{'='*60}\n  inplace+reshard+sync_mode={args.sync_mode}\n{'='*60}")
