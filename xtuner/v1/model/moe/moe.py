        ...
        if self.mtp_block is not None:
            ...
            for mtp_idx, mtp_layer in enumerate(self.mtp_block.layers):
                ...
                if self._should_recompute(None, mtp_idx=mtp_idx) or mtp_config.share_weights:
                    ...
                    #
                    # reentrant 分支用 apply_activation_checkpointing：其内部 _checkpoint_pytree
                    #   已提供 pytree bridge（等价于 PR 旧版 pytree_reentrant_checkpoint 的
                    #   tree_flatten/unflatten + 零元素 grad 入口），并叠加 reuse_during_recompute
                    #   帧；micro2 传入 [embedding_0, embedding_1] 时能逐个 detach 并在 backward
                    #   把梯度交回原图，与 PR Case 2 需求一致。
